...
It'd be great if Azure had the capability of copying managed images from subscription to subscription and region to region, but it doesn't without the OS disk, which gets deleted in the Packer build. I'm not sure why the capability isn't there for Azure (it is with [AWS](https://www.packer.io/docs/builders/amazon-ebs.html#ami_regions)). Either Packer would need to bake into their code to copy the images over _before_ deleting the OS disk or Azure would need to create a way to copy managed images without the OS disk. Regardless, we're kinda stuck doing it this way for now. (09/23/2018)
...
I was still really curious about how you would actually copy the images to other subscriptions just in case we ran across a situation where we couldn't change the image sharing permissions on a subscription or whatever else might come up. I learned that you can use `az image copy` to copy managed images to another region and/or another subscription. However, the thing that makes it super inconvenient is that it relies on the **source OS disk** as the actual source of the copy. Therefore, if you want to copy the managed image to other regions or subscriptions, you will have to have the OS disk from the original VM.

This is important because I'm pretty sure that Packer doesn't give you the option to _not_ delete the source OS disk. It does give you the option to not delete the original VM upon error but not upon a non-erred completion.

The non-Packer automation workflow super sucks, and I'd love to hear from anyone that knows of a better solution. [This guy](https://michaelcollier.wordpress.com/2017/05/03/copy-managed-images/) has a detailed plan lined up, but here's the gist:

1. Create (ARM, PowerShell, Azure CLI, whatever) and configure (Chef, Ansible, etc.) a VM in a subscription however you want.
2. [Sysprep/Generalize](https://docs.microsoft.com/en-us/windows-hardware/manufacture/desktop/sysprep--generalize--a-windows-installation) the VM
3. Don't ever delete the VM / OS disk.
...
All in all, it's better just to use separate builders in the Packer template. Sure it's doing the same thing, but it's simpler to execute and you don't have to have the OS disk laying around forever.
